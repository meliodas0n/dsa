...
def min_count(product_count, box_size):
  if box_size >= product_count: return 1
  l = [i for i in range(1, product_count + 1, box_size)]
  diff = []
  for i, j in pairwise(l):
    diff.append(j - i)
  if sum(diff) == product_count: return len(diff)
  if sum(diff) < product_count: return len(diff) + 1
